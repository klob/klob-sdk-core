...
/**
 * This class is used to collect phone's information.
 */
public class PhoneInfo {
    ...
    public static String getDeviced(Context context) {
        TelephonyManager tm = (TelephonyManager) context.getSystemService(Context.TELEPHONY_SERVICE);
        return tm.getDeviceId();//获取智能设备唯一编号;
    }
    ...
    public void collectDeviceInfo() {
        try {
            ...
            if (pi != null) {
                ...
                mInfo.put("sdk_int", Build.VERSION.SDK_INT + "");
                mInfo.put("sdk", Build.VERSION.SDK);

                mPhotoInfo.put("versionName", versionName);
                mPhotoInfo.put("versionCode", versionCode);
                mPhotoInfo.put("sdk_int", Build.VERSION.SDK_INT + "");
                mPhotoInfo.put("release", Build.VERSION.RELEASE);
                mPhotoInfo.put("CPU_ABI", Build.CPU_ABI);
                mPhotoInfo.put("CPU_ABI2", Build.CPU_ABI2);
                mPhotoInfo.put("brand", Build.BRAND);
                mPhotoInfo.put("model", Build.MODEL);
                mPhotoInfo.put("display", Build.DISPLAY);
                mPhotoInfo.put("device", Build.DEVICE);
                mPhotoInfo.put("manufacturer", Build.MANUFACTURER);
                mPhotoInfo.put("hardware", Build.HARDWARE);
                mPhotoInfo.put("isDebug", BuildConfig.DEBUG + "");
                mPhotoInfo.put("os.arch", System.getProperty("os.arch") + "");
                mPhotoInfo.put("time", longToString(System.currentTimeMillis(), "yyyy-MM-dd HH:mm"));
    ...
    public void print() {
        String info = "";
        for (Map.Entry<String, String> entry : mPhotoInfo.entrySet()) {
            String key = entry.getKey();
            String value = entry.getValue();
            info += key + "=" + value + "\n";
//            Log.e(TAG, info);
